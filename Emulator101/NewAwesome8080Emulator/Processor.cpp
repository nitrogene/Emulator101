...
void Processor::incrementeProgramCounter(const uint8_t* opCodes)
{
	auto opCode = opCodes[0];
	...
	{
		// User manual p59
		// The program counter is not incremented before this instruction
		m_State.PC += isl.Size;
	}
	...
	case 0xC0:
	{
		...
		{
			m_State.Cycles += isl.ClockCycle.B - isl.ClockCycle.A;
		}
		break;
	}
	case 0xC4:
...
void Processor::handleOpcode(const uint8_t* opCodes)
{
...
void Processor::RunStep()
{
	...
	if (!m_Interrupt)
	{
		opCodes = m_MemoryMap.getOpCode(m_State.PC);		
	}
	else
	{
	...
	incrementeProgramCounter(opCodes);
		
	handleOpcode(opCodes);
 ...
 MemoryMap& Processor::getMemoryMap()
{
 ...
 void Processor::setInterrupt(const std::array<uint8_t,3>& opCodes)
 {
	 if (!m_State.INTE)
	 {
		 return;
