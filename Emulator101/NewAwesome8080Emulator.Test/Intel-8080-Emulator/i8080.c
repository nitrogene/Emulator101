...
void UnimplementedInstruction(State8080 *state)
{
	...
	state->pc--;
	Disassemble8080(state->memory, state->pc,state);
	printf("\n");
	exit(1);
}
...
int Disassemble8080(unsigned char *codebuffer, int pc, State8080* state)
{
	...
	printf("\t%02X%02X\t%02X%02X\t%02X%02X\t%02X%02X\t%04X\t%04X\t", state->a, 0, state->b, state->c, state->d, state->e, state->h, state->l, state->pc, state->sp);
	if (state->cc.z)
	{
		printf("Z");
	}
...
void ReadFileIntoMemory(State8080 *state, const char *filename, uint32_t offset)
{
